...
## Instamojo wallet payment setup

The project now uses Instamojo hosted checkout for wallet deposits. The Client ID, Client Secret and Private Salt must be stored in Firebase Secret Manager, never in `app.js` or GitHub.

From the project root, after installing Firebase CLI and selecting the correct Firebase project:

```bash
firebase functions:secrets:set INSTA_CLIENT_ID
firebase functions:secrets:set INSTA_CLIENT_SECRET
firebase functions:secrets:set INSTA_PRIVATE_SALT
firebase deploy --only functions,hosting
```

Use the production credentials from the same `www.instamojo.com` environment as the live API. Test and production credentials are not interchangeable.

The webhook endpoint is automatically configured by the payment request as:
`https://YOUR-FIREBASE-HOSTING-DOMAIN/api/instamojoWebhook`

The player payment endpoint is `/api/createPayment`; successful Instamojo webhooks automatically credit `users/{uid}.walletBalance` and create a `walletTransactions` ledger entry. Duplicate webhook deliveries are handled idempotently by payment ID/status.

Before going live, make sure the Instamojo merchant account is approved for collecting payments.
