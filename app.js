...
async function addMoney(){
 openModal(`<h2>Add Money</h2><p class="muted">Pay securely with Instamojo using UPI, cards or other available payment methods.</p><form id="deposit" class="formgrid">
 <input id="damt" type="number" min="9" max="50000" step="0.01" placeholder="Amount (₹9 minimum)" required>
 <input id="dmob" placeholder="Mobile number" required>
 <input id="demail" type="email" value="${esc(currentUser.email||"")}" placeholder="Email ID" required>
 <button class="btn primary">Pay with Instamojo</button></form>`);
 $("deposit").onsubmit=async e=>{e.preventDefault();const btn=e.target.querySelector("button");btn.disabled=true;btn.textContent="Creating payment...";try{
  const amount=Number($("damt").value);if(amount<9||amount>50000)throw new Error("Amount must be between ₹9 and ₹50,000.");
  const token=await currentUser.getIdToken();
  const r=await fetch("/api/createPayment",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},body:JSON.stringify({amount,mobile:$("dmob").value.trim(),email:$("demail").value.trim()})});
  const x=await r.json();if(!x.success)throw new Error(x.message);
  window.location.href=x.paymentUrl;
 }catch(x){toast(x.message);btn.disabled=false;btn.textContent="Pay with Instamojo"}};
}

async function checkReturnedPayment(){
 const qs=new URLSearchParams(location.search),depositId=qs.get("payment_request_id");
 if(!depositId||!currentUser)return;
 history.replaceState({},document.title,location.pathname+location.hash);
 openModal(`<h2>Payment Processing</h2><p id="paymentMsg" class="muted">We are confirming your Instamojo payment. Please wait...</p>`);
 for(let i=0;i<10;i++){
  try{
   const token=await currentUser.getIdToken();
   const r=await fetch("/api/paymentStatus",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},body:JSON.stringify({depositId})});
   const x=await r.json();
   if(x.success&&x.status==="approved"){toast(`₹${Number(x.amount||0).toFixed(2)} added to wallet.`);closeModal();return;}
   if(x.success&&x.status==="failed"){throw new Error("Payment was not successful.");}
  }catch(e){$("paymentMsg").textContent=e.message||"Payment status could not be checked yet.";return;}
  await new Promise(r=>setTimeout(r,2000));
 }
 $("paymentMsg").textContent="Payment received/processing. Wallet will update automatically after Instamojo confirms it.";
}
